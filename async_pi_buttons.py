...
class AsyncPiButtons:
    ...
    async def __aenter__(self):
        ...
        try:
            self._button_01 = Button(self._button_01_pin,
                                     pull_up=True,
            ...
            logger.exception(f'Failed to attach button_01 interrupt')
            raise
        try:
            self._button_02 = Button(self._button_02_pin,
                                     pull_up=True,
                                     hold_time=self.hold_time,
                                     bounce_time=self.bounce_time)
            ...
            logger.exception(f'Failed to attach button_02 interrupt')
            if self._button_01:
        ...
        if ret:
            with self._lock:
                if not self._double_button_triggered:
                    ...
                    self._loop.call_soon_threadsafe(self._queue.put_nowait, 'double_button')
        else:
